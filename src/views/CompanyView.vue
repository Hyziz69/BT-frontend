    ...
    <div class="company-page">
      ...
      <div v-else>
        <div class="page-header">
          ...
          <button v-if="!isOwner && myRole" @click="handleLeave" class="btn-danger-outline">
            Leave company
          </button>
        </div>
        ...
        <div class="section">
          <div class="section-header">
            <h2>Members</h2>
            <button v-if="isOwner" @click="openInvite" class="btn-primary">
              + Invite colleague
            </button>
          </div>
            ...
            <div v-for="m in members" :key="m.id" class="member-card">
              ...
              <div class="member-right">
                <select
                  ...
                  :value="m.role"
                  @change="handleMemberRoleChange(m, $event)"
                  class="role-select"
                >
          ...
          <div class="members-list">
            <div v-for="inv in invitations" :key="inv.id" class="member-card">
              ...
              <div class="member-right">
                ...
                >
                  {{ cancellingInvitationId === inv.id ? 'Cancelling…' : 'Cancel' }}
                </button>
              </div>
            </div>
          </div>
            ...
            <div v-for="ch in challenges" :key="ch.id" class="challenge-card">
              <div class="challenge-row">
                <div class="challenge-main">
                  <RouterLink :to="`/challenges/${ch.id}`" class="challenge-link">
                    {{ ch.title }}
                  </RouterLink>
                ...
                <div class="challenge-actions">
                  <RouterLink :to="`/challenges/${ch.id}`" class="btn-secondary">
                    Open →
                  </RouterLink>
              ...
              <div v-if="expandedId === ch.id" class="candidates">
                ...
                <div v-else class="candidate-list">
                  <div v-for="app in candidates[ch.id]" :key="app.id" class="candidate-card">
                    ...
                    <span v-else-if="app.status === 'approved'" class="role-badge manager">
                      Selected
                    </span>

                    <span v-else-if="app.status === 'rejected'" class="role-badge pending">
                      Rejected
                    </span>
                  </div>
                </div>
        ...
        <div class="section" v-if="isManager">
          ...
          <div v-else class="challenge-list">
            <div
              v-for="app in acceptedApps"
              :key="app.id"
              class="challenge-card clickable-card"
      ...
      <div v-if="showCreate" class="modal-overlay" @click.self="closeCreate">
        <div class="modal">
          ...
          <div class="field">
            ...
            <textarea
              v-model="createForm.technical_spec"
              rows="4"
          ...
          <div class="field">
            <label>Call *</label>
            <select
              v-model="createForm.call_id"
              class="role-select full"
              :disabled="!selectedProgramId || callsLoading"
            >
      ...
      <div v-if="showInvite" class="modal-overlay" @click.self="closeInvite">
        <div class="modal">
          ...
          <div class="modal-actions">
            <button @click="closeInvite" class="btn-secondary">
...
import { programsApi, type CallOption, type ProgramOption } from '../api/programs'
...
const form = ref<CompanyRegisterPayload>({
  name: '',
  ico: '',
  sector: '',
  website: '',
...
async function handleCancelInvitation(invitation: CompanyInvitation) {
  if (!company.value) return
  ...
  try {
    await companyApi.cancelInvitation(company.value.id, invitation.id)
    await loadMembers(company.value.id)
  } catch (e: any) {
...
function handleMemberRoleChange(member: CompanyMember, event: Event) {
